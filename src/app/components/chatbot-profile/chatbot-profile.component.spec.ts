...
describe('ChatbotProfileComponent', () => {
  ...
  it('should not return when not locale pt-BR and translate when pt-BR', () => {
    expect(component.getCulture('pt-BR')).toEqual('BRA - Portuguese (PT)');
    expect(component.getCulture('en-US')).toEqual();
  });
});
